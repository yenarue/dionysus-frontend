...
export default {
  ...
  data() {
    return {
      displayedFavorite: this.favorited,
      animating: false
    };
  },
  computed: {
    iconClasses() {
      return {
        "toggle-favorite__icon--favorited": this.displayedFavorite,
  ...
  methods: {
    toggle() {
      ...
      if (!this.displayedFavorite) {
        this.animating = true;
      }

      this.displayedFavorite = !this.displayedFavorite;
      this.$emit("toggle", this.displayedFavorite);
    },
    onIconAnimationEnds() {
